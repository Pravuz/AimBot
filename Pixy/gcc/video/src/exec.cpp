...
static const ProcModule g_module[] =
		{
				{ "running", (ProcPtr) exec_running, { END },
						"Is a program running?"
								"@r 1 if a program is running, 2 if running in \"forced\" state, 0 if not" },
				{ "stop", (ProcPtr) exec_stop, { END },
						"Stop execution of the current program"
								"@r always returns 0" }, { "run",
						(ProcPtr) exec_run, { END }, "Run the current program"
								"@r returns 0 if successful, -1 otherwise" }, {
						"runprog", (ProcPtr) exec_runprog, { CRP_UINT8, END },
						"Run the specified program"
								"@p program number"
								"@r returns 0 if successful, -1 otherwise" }, {
						"runprogArg", (ProcPtr) exec_runprogArg, { CRP_UINT8,
						CRP_UINT32, END },
						"Run the specified program with an argument"
								"@p program number"
								"@p argument to be passed to program"
								"@r returns 0 if successful, -1 otherwise" }, {
						"progs", (ProcPtr) exec_list, { END },
						"List available programs"
								"@r always returns 0" },
				{ "version", (ProcPtr) exec_version, { END },
...
int32_t exec_runprog(const uint8_t &progNum) {
	if (progNum != 0
			&& (progNum > EXEC_MAX_PROGS || g_progTable[progNum - 1] == NULL))
		return -1;
...
static void loadParams() {
	...
	prm_add("Default program", 0,
			"Selects the program number that's run by default upon power-up. (default 0)",
			UINT8(0), END);
}
...
void exec_loop() {
	...
	exec_runprog(0); // run prog greyshades.
	//exec_select();
